...
\begin{figure}[h]
  ...
  \caption{Corrélation de chaque variable avec le stress. Barres horizontales triées
    (\up{rouge} = corrélation positive, \down{bleu} = négative), axe partant de zéro,
...
\begin{figure}[h]
  ...
  \caption{Coefficients du modèle (variables standardisées) : poids et signe de chaque
    ...
    business --- qui fait \up{monter} ou \down{baisser} le stress.}
  \label{fig:coefbase}
\end{figure}
...
\begin{tabular}{@{}lcl@{}}
  ...
  \midrule
  \texttt{OVERWORK}       & $\up{+0{,}349}$  & \cmark\ le \textbf{signal le plus fort de toute l'étude} \\
  \texttt{HEALTHY\_HABITS}& $\down{-0{,}218}$& \cmark\ pertinent (habitudes saines $\to$ moins de stress) \\
  \texttt{SLEEP\_x\_FLOW} & $\down{-0{,}157}$& \cmark\ pertinent (l'interaction fonctionne) \\
  \texttt{SOCIAL\_SUPPORT}& $\down{-0{,}055}$& \xmark\ quasi nul --- \textbf{échec instructif} \\
  \bottomrule
\end{tabular}
\end{center}
...
réseau social (\texttt{SOCIAL\_NETWORK}, coef.\ $\up{+0{,}09}$) et le fait d'aider
beaucoup les autres (\texttt{SUPPORTING\_OTHERS}, $\up{+0{,}07}$) vont de pair avec
...
\begin{itemize}[nosep,leftmargin=1.4em]
  ...
  \item \up{\textbf{Font monter}} le stress : \texttt{DAILY\_SHOUTING}
    ($\up{+0{,}32}$), \texttt{LOST\_VACATION} ($\up{+0{,}18}$), \texttt{SOCIAL\_NETWORK}
    ($+0{,}09$), \texttt{PERSONAL\_AWARDS} ($+0{,}06$).
  \item \down{\textbf{Font baisser}} le stress : \texttt{WEEKLY\_MEDITATION}
    ($\down{-0{,}15}$), \texttt{GENDER\_Male} ($-0{,}15$), \texttt{SLEEP\_HOURS}
